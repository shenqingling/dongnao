<template>
  <div class="user_item">
    ...
    <el-table
    :data="posts"
    @selection-change="handlePostsChange"
    stripe
    style="width: 100%">
     <el-table-column
      type="selection"
  ...
  <h3>好友列表</h3>
    ...
    <el-table
    :data="friends"
    @selection-change="handleFriendsChange"
    stripe
    style="width: 100%">
    <el-table-column
      type="selection"
...
<script>
import { detail, addPost, addFriend,removeFriend,removePost,updatePost } from './user.item.api.js';
...
export default {
  data(){
      ...
      removeFriend:{
	    ...
	    "posts": [],
	    "friends": [],
      "dialogTitle": {'true':'编辑文章','false':'新增文章'},
  	}
  },
  methods:{
  	fetch(){
    ...
    handleChangePost(){
      updatePost(this.id, {...this.addPost},(item)=>{
      ...
      removePost(this.id, {...this.removePost},(item)=>{
    ...
    handleRemoveFriend(){
      ...
      removeFriend(this.id,{...this.removeFriend},(item)=>{
        let deletes = removeFriend.split(',');
        for(let i = 0;i<deletes.length;i++){
